...
std::string convertToString(const FeatureType val)
{ return featureType(val); }
...
void convertFromString(const std::string& str, FeatureType& val)
{
  ...
  for (int i = 0; i < NFEATURE_TYPES; i ++)
    if (str.compare(featureType(FeatureType(i))) == 0)
      { val = FeatureType(i); return; }

  conversion_error::raise<FeatureType>(str);
}
